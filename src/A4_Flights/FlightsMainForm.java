...
public class FlightsMainForm extends javax.swing.JFrame implements ActionListener {
    ...
    private void btnQueryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnQueryActionPerformed
        ...
        Object queryObj = comboQuery.getSelectedItem();
        Object paramObj = comboParameter.getSelectedItem();
        Object formatObj = comboFormat.getSelectedItem();

        // Check if any of the combo boxes have null values
        if (paramObj == null) {
            paramObj="";
        }

        // Convert the selected items to Strings
        ...
        txtOutput.append("# Flights = "+flights.length);
...
private void btnReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportActionPerformed
    ...
    private void reportSummaryByFlightType() {
        ...
        String[] flightTypes = {Common.CARGO, Common.PASSENGER, Common.TRAINING};

        for (String flightType : flightTypes) {
            Flight[] flights = schedule.getFlightsByType(flightType);
            count = flights.length;

            int insideweight=0;
            for (Flight flight : flights) {
                insideweight += flight.calculateWeight();

            }
            bigWeight+=insideweight;
            txtOutput.append("  "+count+" "+ flightType +" Flights:\t weight = "+Common.format(insideweight)+"\n");
        }
        txtOutput.append("---------------------------------------------------------------------\n");
        txtOutput.append(allFlights.length+"  flights " + "  \t\tweight = "+ Common.format(bigWeight) + "\n");
    }
    ...
    private void reportSummaryByDestination() {
        ...
        for (Location location : locations) {
            ...
            Flight[] flights = schedule.getFlightsByDestination(locationCode);
            int count = flights.length;
            int totalWeight = 0;

            for (Flight flight : flights) {
                totalWeight += flight.calculateWeight();
            }

            if (count > 0) {
                txtOutput.append(count+"  flights to "+ locationCode + "  \tweight = "+ Common.format(totalWeight) + "\n");
                bigWeight+=totalWeight;
            }
        }
        txtOutput.append("---------------------------------------------------------------------\n");
        txtOutput.append(allFlights.length+"  flights " + "  \t\tweight = "+ Common.format(bigWeight) + "\n");
    ...
    private void outputFlightsInArchiveFormat(Flight[] flights) {
        ...
        txtOutput.setText("");
        for (Flight flight : flights)
            txtOutput.append(flight.getFlightType()+","+flight.toArchiveFormat() + "\n");
    }
    ...
    private boolean createFlights(String fileName) {
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                Location destination = findLocation(parts[4]);
                int numCrew = Integer.parseInt(parts[5]);

                Flight flight;
                if (flightType.equalsIgnoreCase(Common.CARGO)) {
                    int cargoWeight = Integer.parseInt(parts[6]);
                    flight = new CargoFlight(flightNumber, dayOfWeek, departureTime, destination, numCrew, cargoWeight);
                } else if (flightType.equalsIgnoreCase(Common.PASSENGER)) {
                    int numPassengers = Integer.parseInt(parts[6]);
                    flight = new PassengerFlight(flightNumber, dayOfWeek, departureTime, destination, numCrew, numPassengers);
                } else if (flightType.equalsIgnoreCase(Common.TRAINING)) {
                    flight = new TrainingFlight(flightNumber, dayOfWeek, departureTime, destination, numCrew);
                } else {
                    System.out.println("Unknown flight type: " + flightType);
                    continue;
                }
            ...
            System.out.println(result.size() + " Flights loaded");
            this.allFlights = result.toArray(new Flight[0]);
            return true;
        } catch (Exception ex) {
    ...
    private void runPolicyCheckReport() {
        // Get all flights from the schedule
        Flight[] flights = schedule.getAllFlights();

        // Run the policy checks on the flights
        String[] results = PolicyChecker.runPolicyChecks(flights);
        int counter = 0;

        // Loop through the flights and check for errors
        for (int i = 0; i < flights.length; i++) {
            if (!results[i].equals(Common.NO_ERROR)) {
                // Display the flight in archive format and the errors, if it works don't change it (the rule of life)
                txtOutput.append(flights[i].getFlightType()+","+flights[i].toArchiveFormat() + "\n");
                txtOutput.append(results[i] + "\n\n");
                counter++;
            }
        }
        txtOutput.append(counter+" Errors were found");
    }
